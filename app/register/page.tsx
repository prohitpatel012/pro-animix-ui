...
import { Loader2, Eye, EyeOff, CheckCircle, AlertCircle, ArrowRight, Sparkles } from "lucide-react";
...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                // Handle Zod server errors if structured
                if (data.errors) {
                    const serverErrors: Partial<RegisterFormData> = {};
                    // Map Zod format (data.errors.field._errors[0])
                    if (data.errors.name?._errors?.[0]) serverErrors.name = data.errors.name._errors[0];
                    if (data.errors.email?._errors?.[0]) serverErrors.email = data.errors.email._errors[0];
                    if (data.errors.password?._errors?.[0]) serverErrors.password = data.errors.password._errors[0];
    ...
    return (
        <div className="min-h-screen w-full flex bg-white dark:bg-black overflow-hidden">
            {/* Left Side - Visuals (40-50% width) */}
            <div className="hidden lg:flex w-1/2 relative bg-zinc-900 text-white overflow-hidden p-12 flex-col justify-between">
                {/* Abstract Background */}
                <div className="absolute inset-0 z-0">
                    <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_right,var(--tw-gradient-stops))] from-indigo-500/30 via-zinc-900 to-zinc-950" />
                    <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-purple-500/20 blur-[120px] rounded-full mix-blend-screen" />
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-indigo-500/10 blur-[100px] rounded-full animate-pulse" />
                ...
                <div className="relative z-10 max-w-lg">
                    <h2 className="text-4xl font-bold leading-tight mb-6 text-transparent bg-clip-text bg-linear-to-b from-white to-white/60">
                        Build your next big idea with premium components.
                    </h2>
                    <p className="text-zinc-400 text-lg leading-relaxed mb-8">
                        Join thousands of developers building stunning applications 10x faster with our expertly crafted UI library.
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className={`w-full px-4 py-3 rounded-lg bg-neutral-50 dark:bg-neutral-900 border ${errors.name ? "border-red-500 focus:ring-red-500" : "border-neutral-200 dark:border-neutral-800 focus:ring-black dark:focus:ring-white"
                                ...
                                >
                                    ...
                                    <div className="flex gap-1 h-1">
                                        {[...Array(4)].map((_, i) => (
                                            <div
                                                key={i}
                                                className={`h-full w-full rounded-full transition-colors duration-300 ${i < strength
                                                    ? strength <= 2 ? "bg-orange-500" : "bg-green-500"
                                    ...
                                    <ul className="space-y-1.5">
                                        {requirements.map((req, i) => (
                                            <li key={i} className="flex items-center gap-2 text-xs">
                                                <div className={`w-4 h-4 rounded-full flex items-center justify-center transition-colors ${req.met
                                                    ? "bg-green-500/10 text-green-500"
                                                    : "bg-neutral-100 dark:bg-neutral-800 text-neutral-400"
                                                    }`}>
                                                    {req.met ? <CheckCircle size={10} /> : <div className="w-1 h-1 rounded-full bg-neutral-400" />}
                                                </div>
                                                <span className={`transition-colors ${req.met ? "text-neutral-900 dark:text-neutral-100 font-medium" : "text-neutral-500"
                                                    }`}>
                                                    {req.label}
                                                </span>
                                            </li>
